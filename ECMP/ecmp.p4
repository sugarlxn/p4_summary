    ...
    out ingress_intrinsic_metadata_t  ig_intr_md)
{
    ...
    state parse_udp {
      meta.ll=pkt.lookahead<bit<32>>();
        pkt.extract(hdr.udp);
        meta.sport=hdr.udp.src_port;
        meta.dport=hdr.udp.dst_port;
        transition accept;
    }
...
action cal_ecmp()//计算index
    {
        ecmp=hash_ecmp.get({hdr.ipv4.src_addr,hdr.ipv4.dst_addr,meta.ll,hdr.ipv4.protocol,meta.sport,meta.dport});
    }
